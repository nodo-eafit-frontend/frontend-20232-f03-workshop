...
app.get('/recipe', (req, res) => {
  //recipe es la ruta. (req, res) es el handler
  res.status(200).send({
    //primero se manda un estado para determinar si success o failure
...
app.post('/recipe/:id', (req, res) => {
  ...
  if (!title) {
    //si no hay titulo, mandar estado de 400 y mandar error message
    return res.status(400).send({ error: 'title is required' });
  }

  res.send({
    //si hay un titulo, mandar recipe
