...
function getRadioResult(element, name) {
  return element.querySelector(`input[name="${name}"]:checked`)?.value;
>>>>>>> fc3687920e7e20886fa324bc7fdd37a3352d333e
}
...
function onSubmit(event) {
  ...
  if (recipe.numberPeople.min > recipe.numberPeople.max) {
    validation.isValid = false;
    validation.message =
      'por favor chequear que el numero de min y max sigan la siguiente logica:  el minimo siempre debe ser menor que el maximo';
  }
